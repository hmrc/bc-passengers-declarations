...
class PaymentTimeoutWorkerISpec
  ...
  lazy val validationService: ValidationService           = app.injector.instanceOf[ValidationService]
  implicit lazy val mat: Materializer                     = app.injector.instanceOf[Materializer]
  lazy val chargeReferenceService: ChargeReferenceService = app.injector.instanceOf[ChargeReferenceService]
  ...
  override def fakeApplication(): Application = builder.build()
  ...
  "A payment timeout worker" should {
    ...
    "must continue to process data" in {
      ...
      await(worker.tap.pull()) shouldBe defined
      await(worker.tap.pull()) shouldBe defined
