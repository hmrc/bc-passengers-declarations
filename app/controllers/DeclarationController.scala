...
class DeclarationController @Inject() (
  ...
  def submitAmendment(): Action[JsValue] = Action.async(parse.tolerantJson) { implicit request =>
    request.headers.get(CorrelationIdKey) match {
      case Some(cid) =>
        ...
        if (validationErrors.isEmpty) {
          val id              =
            (amendmentData \ "simpleDeclarationRequest" \ "requestDetail" \ "declarationHeader" \ "chargeReference")
              .as[String]
          val chargeReference =
            ChargeReference(id).getOrElse(throw new Exception(s"unable to extract charge reference:$id"))
          withLock(chargeReference) {
            repository
              .insertAmendment(amendmentData, cid, chargeReference)
              .map(declaration => Accepted(declaration.amendData.get).withHeaders(CorrelationIdKey -> cid))
          }
        } else {
  ...
  def retrieveDeclaration(): Action[JsValue] = Action.async(parse.tolerantJson) { implicit request =>
    request.body
      .validate[PreviousDeclarationRequest]
      .map { retrieveDeclarationRequest =>
        repository.get(retrieveDeclarationRequest).flatMap {
          _.map { declarationResponse =>
  ...
  private def updateAmendState(declaration: Declaration, paymentNotification: PaymentNotification)(implicit
    hc: HeaderCarrier
  ): Future[Status] =
  ...
  private def withLock(chargeReference: ChargeReference)(f: => Future[Result]): Future[Result] =
    lockRepository.lock(chargeReference.value).flatMap { gotLock =>
      if (gotLock) {
        f.flatMap { result =>
          lockRepository.release(chargeReference.value).map { _ =>
            result
          }
        }.recoverWith { case e =>
          lockRepository
            .release(chargeReference.value)
            .map(_ => throw e)
        }
      } else {
